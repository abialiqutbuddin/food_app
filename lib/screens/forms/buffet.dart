...
class BuffetForm extends StatefulWidget {
  ...
  const BuffetForm({super.key, required this.cateringId});

  @override
  State<BuffetForm> createState() => _BuffetFormState();
}
...
class _BuffetFormState extends State<BuffetForm> {
  ...
  void _save() {
    ...
    // Attach all selected menu items (no size, no per-item price)
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Buffet order added')),
    );
  }
